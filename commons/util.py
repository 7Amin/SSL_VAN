...
def fix_outputs_url(args, base_url):
    ...
    print(f" output_url 1 is {output_url}")
    print(f" exists 1 is {os.path.exists(output_url)}")
    if not os.path.isdir(output_url):
        os.mkdir(output_url)
    output_url = output_url + "/" + base_url
    print(f" output_url 2 is {output_url}")
    print(f" exists 2 is {os.path.exists(output_url)}")
