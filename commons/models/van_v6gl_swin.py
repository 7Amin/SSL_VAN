...
class VANV6GLSwin(nn.Module):
    def __init__(self, embed_dims, mlp_ratios, depths, num_stages, in_channels, out_channels, dropout_path_rate,
        ...
        super(VANV6GLSwin, self).__init__()
        self.patch_count = patch_count
        for i in range(patch_count):
            for j in range(patch_count):
                for k in range(patch_count):
                    setattr(self, f"van{i}_{j}_{k}", VANV6Swin(embed_dims[:-2], mlp_ratios[:-2], depths[:-2],
                                                           num_stages - 2, in_channels, out_channels,
                                                           dropout_path_rate, upsample))
        self.van = VANV6Swin(embed_dims, mlp_ratios, depths, num_stages, in_channels,
