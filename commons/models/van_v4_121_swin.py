...
from commons.models.util_models.van_3d_swin import VANSwin3D


class VANV4121Swin(nn.Module):
    def __init__(self, embed_dims, mlp_ratios, depths, num_stages, in_channels, out_channels, dropout_path_rate,
                 upsample="deconv"):
        super(VANV4121Swin, self).__init__()
        self.van3d = VANSwin3D(in_chans=in_channels, drop_path_rate=dropout_path_rate, embed_dims=embed_dims,
        ...
        for i in range(num_stages - 1):
            conv = nn.Sequential(
                ...
                nn.BatchNorm3d(embed_dims[-1] // 2 ** (i + 1)),
                nn.LeakyReLU(),
        ...
        self.first_conv = nn.Sequential(
            ...
            nn.Conv3d(embed_dims[-1] // 2 ** num_stages, embed_dims[-1] // 2 ** (num_stages - 1),
                      kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(embed_dims[-1] // 2 ** (num_stages - 1)),
            nn.LeakyReLU()
            )
        ...
        for i in range(num_stages):
            upsample = nn.Sequential(
                ...
                nn.BatchNorm3d(embed_dims[-1] // 2**(i + 1)),
                nn.LeakyReLU(),
                nn.Conv3d(embed_dims[-1] // 2 ** (i + 1), embed_dims[-1] // 2 ** (i + 1), kernel_size=3, stride=1,
                          padding=1),
                nn.BatchNorm3d(embed_dims[-1] // 2 ** (i + 1)),
                nn.LeakyReLU(),
