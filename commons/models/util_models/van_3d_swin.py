...
import math
import warnings
from commons.models.util_models.attentions.LKA3D import SpatialAttention3D
...
class SwinBlock3D(nn.Module):
    def __init__(self,
                 ...
                 norm_cfg=dict(type='SyncBN', requires_grad=True)):
        super().__init__()
        self.b1 = Block3D(dim=dim,
                         mlp_ratio=mlp_ratio,
                         drop=drop,
                         drop_path=drop_path,
                         linear=linear,
                         norm_cfg=norm_cfg)
        self.b2 = Block3D(dim=dim,
                    mlp_ratio=mlp_ratio,
                    drop=drop,
                    drop_path=drop_path,
                    linear=linear,
...
class VANSwin3D(nn.Module):
    def __init__(self,
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([SwinBlock3D(dim=embed_dims[i],
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            super(VANSwin3D, self).init_weights()
