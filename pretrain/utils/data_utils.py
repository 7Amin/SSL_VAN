...
def get_loader(args):
    ...
    train_transform = SimMIMTransform(config)
    val_transform = SimMIMTransform(config)

    if args.cache_dataset:
        print("Using MONAI Cache Dataset")
        train_ds = CacheDataset(data=datalist, transform=train_transform, cache_rate=0.5, num_workers=num_workers)
